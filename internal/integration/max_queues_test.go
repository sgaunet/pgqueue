...
func TestCreateQueue_MaxQueuesLimit(t *testing.T) {
	...
	for i := range 2 {
		...
		if err := pq.CreateChannel(ctx, name, pgqueue.ChannelOptions{}); err != nil {
			t.Fatalf("CreateChannel(%s) failed within quota: %v", name, err)
		}
	}
	if err := pq.CreateTopic(ctx, "max-topic-0", pgqueue.TopicOptions{}); err != nil {
		t.Fatalf("CreateTopic failed within quota: %v", err)
	}

	// The next creation must be rejected.
	err = pq.CreateChannel(ctx, "max-ch-overflow", pgqueue.ChannelOptions{})
	...
	err = pq.CreateTopic(ctx, "max-topic-overflow", pgqueue.TopicOptions{})
	if !errors.Is(err, pgqueue.ErrMaxQueuesReached) {
	...
	if err := pq.CreateChannel(ctx, "max-ch-after-delete", pgqueue.ChannelOptions{}); err != nil {
